...
this will slide up and down then animate when clicked
  * 
  * 
 $("button").on("click", function () {
    $("h1").animate({ margin: "20%", opacity:"0.5"});
});
more values can be added to animation by use coma
  * 
  $("button").on("click", function () {
    $("h1").animate({opacity: 0.5});
});
animate can change the style but only in numeric values, anything that involves a value with text
wont be accepted
  * 
 $("button").on("click", function () {
    $("h1").slideUp();
});
this will slide buttons up and slideToggle will slide them up and down
  * 
  $("button").on("click", function () {
    $("h1").fadeOut();
});
fadeout will fade the h1 slowly and fadeToggle will fadeout and in
  * 
  * 
  * 
 $("button").on("click", function () {
...
toggle hides and unhides every time its clicked
...
This is a the mouseover function with on functionality
  * 
  * 
 $("h1").on("click", function () {
    $("h1").css("color", "red");
});
This function is best because you can add any functionality such as click being replaced with
keypress, mouseover and whatever command available
  * 
  $("input").keypress(function () {
    var data = event.key;
    $("h1").text(data);
});
keypress functionality is same as keypress but no need for loop
keypress is functional in input
  * 
  $("button").click(function () {
    $("h1").css("color", "purple");
});
this is the same choosing all buttons in javascript but easier since in javascript for loop is required
to choose all buttons
  * 
  * 
  * 
  $("h1").click(function(){
...
this will change color to red if clicked, this is the same as addEventListener("click", function)

  * 
  * 
 $("img").attr("class");
this function will find all classes inside image
 * 
$("a").attr("href", "https://www.yahoo.com")
This will change the url attribute similar to setAttribute


 $("img").attr("src", "images/kick.png")
this will change the src attribute similar to setAttribute
 * 
 $("h1").html("<em>Hi</em>");
$("button").html("<em>Dont Click Me</em>");
.html is the same as innerHTML which you can add structure inside text
 * 
 * 
 $("h1").text("Hi");
this will change text same as textContent in javascript
 
$("h1").addClass("jqueryh1");
this will add class
$("h1").removeClass("jqueryh1");
this will remove Class
$("h1").addClass("jqueryh1 margin-50");
this will add multiple classes

 * 
 $("h1").css("font-size");
 This command will show how much is the size of h1
 * 
 * 
 $("button");
This select all buttons like document.querySelectorAll
 * 
 minifier.org can make your code shorter if necessary
 * 
 $("body").css("background-color", "orange");
Everything can be changed same as javascript including document functions
 * 
 * 
 $("h1").css("color","red");
jquery color changes the color of h1 similar to javascript but less codes to run
 * 

var data = ["images/dice1.png", "images/dice2.png", "images/dice3.png", "images/dice4.png", "images/dice5.png", "images/dice6.png"];
var randomdice = (Math.floor(Math.random() * 5) + 1);
var randomdice1 = (Math.floor(Math.random() * 5) + 1);
var a;
var b;
function getdata(data, randomdice, randomdice1) {
    a = data[(randomdice - 1)];
    document.querySelector(".image1").setAttribute("src", a);
    b = data[(randomdice1 - 1)];
    document.querySelector(".image2").setAttribute("src", b);
...
if (randomdice > randomdice1) {
    ...
    document.querySelector(".left").classList.add("unhide");
    getdata(data, randomdice, randomdice1);

}
else if (randomdice < randomdice1) {
    document.querySelector("h1").textContent = "Player 2 wins";
    document.querySelector(".right").classList.add("unhide");
    getdata(data, randomdice, randomdice1);
}
else {
...
This is the code for the dice game
 * 
 * 
