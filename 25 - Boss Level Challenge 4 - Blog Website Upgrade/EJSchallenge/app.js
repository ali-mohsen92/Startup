...
mongoose.connect("mongodb://localhost:27017/DatabaseBlog");
const postArraysSchema = new mongoose.Schema({
    title: String,
    content: String
});
const newArray = mongoose.model("newArray", postArraysSchema);
...
app.get("/posts/:postName", function (req, res) {
    ...
    var a;
    newArray.findOne({ _id: requestedid }, function (err,founditem) {
        res.render("post", { postTitle: founditem.title, postContent: founditem.content });
    });

});
    /*newArray.find({}, function (err, founditems) {
        founditems.forEach(function (founditem) {
            console.log(founditem._id);
            console.log(requestedid);
            if (founditem._id === requestedid) {
                res.render("post", { postTitle: founditem.title, postContent: founditem.content });
                a = "Match Found!";
            }
        });
...
app.get("/", function (req, res) {
    newArray.find({}, function (err, foundItems) {
        if (err) {
            ...
            res.render("home", { homeStartingContent: homeStartingContent, Arrays: foundItems });
        }
    });
...
app.post("/compose", function (req, res) {
    const post = new newArray({ title: req.body.postTtitle, content: req.body.postBody });
    post.save();
    res.redirect("/");
